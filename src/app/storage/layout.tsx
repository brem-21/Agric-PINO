...
export default async function StorageLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!session || (session.user.role !== "STORAGE_FACILITY" && session.user.role !== "ADMIN")) {
    redirect("/auth/login");
  }

  return (
    <div className="min-h-screen bg-[#fcfcf7]">
      <SessionGuard expectedRoles={["STORAGE_FACILITY", "ADMIN"]} />
      <PresenceTracker isRider={false} />
      <SidebarCollapseProvider>
        <StorageSidebar user={{ name: session.user.name ?? "Facility Operator", phone: session.user.phone ?? "", image: session.user.image }} isIncidentTeam={session.user.isIncidentTeam} />
