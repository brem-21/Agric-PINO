...
import { TransportPaymentPanel } from "@/components/shared/transport-payment-panel";
...
function StorageTransportContent() {
  ...
  // A typed-only address (no GPS, no map pin) otherwise never gets coordinates,
  // which silently hides the fare/ETA estimate below — resolve it via geocoding
  // as soon as the field loses focus, same as picking it on the map would.
  const geocodeAddress = useCallback(async (field: "pickup" | "delivery", address: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (result) {
          if (field === "pickup") { setPickupLat(result.lat); setPickupLng(result.lon); }
          else { setDeliveryLat(result.lat); setDeliveryLng(result.lon); }
        }
      }
    } finally {
      setGeocoding(null);
    ...
    Promise.all([
      ...
      .then(([orderData, profileData]) => {
        ...
        if (order.buyer.latitude && order.buyer.longitude) {
          ...
          // Most buyers have no stored GPS coordinates — geocode the
          // address text we just prefilled so the fare estimate still shows.
          geocodeAddress("delivery", buyerLocation);
        }
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="space-y-2">
                ...
                />
                {geocoding === "pickup" && (
                  <p className="text-xs text-[#1c3a13]/40 flex items-center gap-1"><Loader2 className="h-3 w-3 animate-spin" />Locating address…</p>
                )}
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onBlur={(e) => { if (!deliveryLat) geocodeAddress("delivery", e.target.value); }}
                  required
                  className="bg-[#fcfcf7] border-[#eeeee9] text-[#1c3a13] rounded-lg"
                />
                {geocoding === "delivery" && (
