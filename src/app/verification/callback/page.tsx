      ...
      <div className="max-w-sm w-full bg-[#fcfcf7] rounded-2xl border border-[#eeeee9] p-8 text-center space-y-4">
        ...
        {state === "success" && (
          <>
            ...
            <p className="text-sm text-[#1c3a13]/50">Your verification request has been submitted and is awaiting admin review.</p>
            <Button onClick={() => router.push("/verification")} className="w-full bg-[#1c3a13] text-[#fcfcf7] rounded-full hover:bg-[#2a5219]">
              View Status
            </Button>
          </>
        )}
        {state === "failed" && (
          <>
            ...
            <Button onClick={() => router.push("/verification")} variant="outline" className="w-full rounded-full border-[#eeeee9] text-[#1c3a13]">
