...
export default async function VendorLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!session || (session.user.role !== "VENDOR" && !session.user.isVendor && session.user.role !== "ADMIN")) {
    redirect("/auth/login");
  }

  return (
    <div className="min-h-screen bg-[#fcfcf7]">
      <SessionGuard expectedRoles={["VENDOR", "ADMIN"]} allowVendorFlag />
      <PresenceTracker isRider={false} />
      <SidebarCollapseProvider>
        <VendorSidebar user={{ name: session.user.name ?? "Vendor", phone: session.user.phone ?? "", image: session.user.image }} />
