...
export default function AdminRequestsPage() {
  ...
  const fetchRows = useCallback(async () => {
    ...
    const res = await fetch(`/api/admin/admin-requests?${params}`);
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-light tracking-tight text-[#1c3a13]">Admin Applications</h1>
        <p className="text-sm text-[#1c3a13]/50 mt-1">{pagination.total} applications</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-[#fcfcf7] rounded-2xl border border-[#eeeee9] py-20 text-center">
          ...
          <p className="text-[#1c3a13]/50">{tab === "PENDING" ? "No admin applications pending review." : "No applications found."}</p>
        </div>
      ) : (
                ...
                <div className="flex flex-col sm:flex-row gap-4 p-5">
                  <div className="flex gap-2 flex-shrink-0">
                    <button onClick={() => setLightbox(r.idPhotoFront)} className="h-20 w-28 rounded-xl overflow-hidden bg-[#eeeee9] border border-[#eeeee9] flex items-center justify-center">
                      ...
                      <img src={r.idPhotoFront} alt="Ghana Card" className="h-full w-full object-cover" />
                    </button>
                    {r.idPhotoBack && (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-1">
                      ...
                      <span className="rounded-full px-2 py-0.5 text-xs font-medium bg-[#eeeee9] text-[#1c3a13]">ADMIN</span>
                    ...
                    <div className="text-xs text-[#1c3a13]/50 mt-1.5 space-y-0.5">
                      ...
                      <p>Applied {formatDate(r.createdAt)}</p>
